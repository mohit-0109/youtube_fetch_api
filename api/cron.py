...
class CallYoutubeApi(CronJobBase):
    RUN_EVERY_MINS = 4  # runs after every 4 minutes
    ...
    def do(self):
        ...
        last_request_time = time_now - timedelta(minutes=4)
        # A variable to check if a valid api key exists or not
        valid = False
        #looping through all the api keys present in settings file
        for apiKey in apiKeys:
            try:
                ...
                if not(code == 400 or code == 403): #check if valid api 
                    break
